...
int recurse(int *arr,int l,int h){
    if(l==h)
        {
            return arr[l];
    ...
    return(max(arr[l]+min(recurse(arr,l+2,h),recurse(arr,l+1,h-1)),arr[h]+min(recurse(arr,l+1,h-1),recurse(arr,l,h-2))));
